...
void StateTopBar::Start()
{
	...
	m_TopBar->TransCenterSetting(L"PlayerStateBar.bmp", { 1280, 50 });
	...
	m_iPlayerHP = GlobalValue::MainPlayer->m_PlayerInfo.GetCurHP();
	m_iPlayerMaxHP = GlobalValue::MainPlayer->m_PlayerInfo.GetMaxHP();
	m_iPlayerMP = GlobalValue::MainPlayer->m_PlayerInfo.GetCurMP();
	m_iPlayerMaxMP = GlobalValue::MainPlayer->m_PlayerInfo.GetMaxMP();
	m_iPlayerHoldingGold = GlobalValue::MainPlayer->m_PlayerInfo.GetHoldingGold();
	...
	for (int i = 0; i < m_iPlayerMaxHP / 2; i++)
	{
		...
		PlayerHPRenderer->TransCenterSetting(L"PlayerHP.bmp", { 40,38 });
	...
	// 골드량
	m_iPlayerHoldingGold = GlobalValue::MainPlayer->m_PlayerInfo.GetHoldingGold();
	m_iPlayerMaxHoldingGold = GlobalValue::MainPlayer->m_PlayerInfo.GetMaxHoldingGold();
	...
	for (int i = 0; i < GoldTextLen; i++)
	{
		...
		PlayerGoldRenderer->TransCenterSetting(L"NumberText.bmp", { 60, 20 });
	...
	for (int i = 0; i < MPTextLen; i++)
	{
		...
		PlayerMPRenderer->TransCenterSetting(L"NumberTextColor.bmp", { 60, 20 });
	...
	m_iFinalBossHP = GlobalValue::FinalBoss->m_BlackKnightInfo.GetHP();
	m_iFinalBossMaxHP = GlobalValue::FinalBoss->m_BlackKnightInfo.GetMaxHP();
	...
	for (int i = 0; i < m_iFinalBossMaxHP / 2; i++)
	{
		...
		FinalBossHPRenderer->TransCenterSetting(L"FinalBossHP.bmp", { 40,38 });
		FinalBossHPRenderer->SetPivotPos(FLOAT4(1080.f + (36.f * i), 34.f));
...
void StateTopBar::Update()
{
	// 플레이어 체력 갱신
	int PlayerHP = GlobalValue::MainPlayer->m_PlayerInfo.GetCurHP();
	int PlayerMaxHP = GlobalValue::MainPlayer->m_PlayerInfo.GetMaxHP();
	...
	// 플레이어 골드량 갱신
	int PlayerGold = GlobalValue::MainPlayer->m_PlayerInfo.GetHoldingGold();
	...
	// 플레이어 마력 갱신
	int PlayerMP = GlobalValue::MainPlayer->m_PlayerInfo.GetCurMP();
	...
	// 보스 체력갱신
	FLOAT4 BossHPCreatePos = GetScene()->GetCamPos() - GlobalValue::FinalBoss->GetPosition();
	if (1280.0f >= FLOAT4::Len2D(BossHPCreatePos))
	{
		int FinalBossHP = GlobalValue::FinalBoss->m_BlackKnightInfo.GetHP();
		int FinalBossMaxHP = GlobalValue::FinalBoss->m_BlackKnightInfo.GetMaxHP();
